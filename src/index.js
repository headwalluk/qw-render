...
const self = {
  ...
  filterNames: [
    'renderedTextContent', //  Rendered text content that can be put into the cache - never add dynamic content with this filter
    'outputHtml', // Sent to the browser - can include dyanmic per-request content
  ],
  ...
  minifyConent: (content, params) => {
    if (!self.config.minify.isEnabled) {
      // ...
    } else if (Array.isArray(self.config.minify.contentTypes) && !self.config.minify.contentTypes.includes(params.contentType)) {
      // ...
    } else if (params.fileName.includes('.min.')) {
      // ...
    } else {
      // Instantiate on first use
      if (!self.minifier) {
        self.minifier = require('html-minifier');
      }
      content = self.minifier.minify(content, self.config.minify.options);
    }

    return content;
  },

  sendFile: (req, res, fileName = null) => {
    if (!fileName) {
      const url = URL.parse(`${req.protocol}://${req.hostname}${req.originalUrl}`);
      fileName = url.pathname;
    }

    if (!fileName || (fileName.endsWith('/') && self.config.defaultIndex)) {
      fileName = path.join(fileName, self.config.defaultIndex); // `${fileName}${self.config.defaultIndex}`;
    }

    let fullPath = self.findStaticFile(fileName);

    // Convert locations like /something to /something/index.html
    if (fullPath && fs.existsSync(fullPath) && fs.statSync(fullPath).isDirectory() && self.config.defaultIndex) {
      fileName = path.join(fileName, self.config.defaultIndex); // `${fileName}${self.config.defaultIndex}`;
      fullPath = self.findStaticFile(fileName);
    }

    let contentType = mime.lookup(fileName);
    let contentBody = null;
    const isText = contentType && (contentType.startsWith('text/') || contentType == 'application/javascript');
    ...
    if (self.fileCache && self.fileCache[fullPath]) {
      ...
      if (rawHtml) {
        ...
        if (isText) {
          contentBody = self.applyFilters('renderedTextContent', contentBody, filterParams);
        }
      }
